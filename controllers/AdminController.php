...
function addNewCatAction()
{
  ...
  if ($res) {
    ...
    $resData['success'] = 0;
    $resData['message'] = 'Ошибка добовления категории';
  }
...
{
  $rsCategories = getAllCategoriy();
  $rsMainCtegories = getAllMainCategories();

  $smarty->assign('rsCategories', $rsCategories);
  $smarty->assign('rsMainCategories', $rsMainCtegories);
  $smarty->assign('pageTitle', 'Управление сайтом');
...
function updatecategoryAction()
{
  ...
  if ($res) {
    ...
    $resData['success'] = 0;
    $resData['message'] = 'Ошибка изминения категории';
  }
...
function addProductAction()
{
  ...
  if ($res) {
    $resData['success'] = 1;
    $resData['message'] = 'Изменение успешно внесено';
  } else {
    $resData['success'] = 0;
...
function updateProductAction()
{
  ...
  if($res){
    $resData['success'] = 1;
    $resData['message'] = 'Изминение успешно сохранено';
  } else {
    $resData['success'] = 0;
    $resData['message'] = 'Ошибка изминение данных';
  }
...
function updateorderaatepaymentAction(){
  $itemId = $_POST['itemId'];
